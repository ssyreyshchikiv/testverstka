$(document).ready(function(){
	...
	$(".progressbar_risk li").on("click", function(){
		var $this = $(this);
		var $modal = $("#modal");
		var $trigger = false;
		// Ищем элементы с классом фолс, чтобы запретить дальнейшую обработку
		$(".progressbar_risk li").each(function(){
		...
		if($(this).prev().hasClass("progresel_risk") && 
			!$(this).prev().hasClass("complete") ||
			$(this).prev().hasClass("preparing")){
			return false;
		}
		if(!$(this).next().hasClass("progresel_risk")){
			return false;	
		}
...
var eventMy = function(step, action){
	...
	switch(action){
		...
		case "2":
			$classSt  = "preparing";
			$classCat = "circle-risk-color-orange";
			break;
		case "3":
			$classSt  = "fail";
			$classCat = "circle-risk-color-red";
			break;
		default:
