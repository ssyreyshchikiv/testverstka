$(document).ready(function(){
	...
	$(".progressbar_gos li").on("click", function(){
		var $this = $(this);
		var $modal = $("#modal");
		var $trigger = false;
		// Ищем элементы с классом фолс, чтобы запретить дальнейшую обработку
		$(".progressbar_gos li").each(function(){
		...
		if($(this).prev().hasClass("progresel_gos") && 
			!$(this).prev().hasClass("complete") ||
			$(this).prev().hasClass("preparing")){
			return false;
		}
		if(!$(this).next().hasClass("progresel_gos")){
			return false;	
		}
...
var eventMy = function(step, action){
	...
	switch(action){
		...
		case "2":
			$classSt  = "preparing";
			$classCat = "circle-gos-color-orange";
			break;
		case "3":
			$classSt  = "fail";
			$classCat = "circle-gos-color-red";
			break;
		default:
